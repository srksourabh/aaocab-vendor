    ...
    (
      ...
      const interval = setInterval(() => {
        ...
        if (progress >= 100) {
          ...
          // Start AI scan — send file as FormData (blob URLs can't be fetched server-side)
          aiSetter((prev) => ({ ...prev, scanning: true }));
          const fd = new FormData();
          fd.append("file", file);
          fd.append("documentType", docType);
          fetch("/api/vendor/parse-document", { method: "POST", body: fd })
            .then((res) => res.json())
            .then((extracted) => {
              aiSetter((prev) => ({
                ...prev,
            ...
            .catch(() => {
              aiSetter((prev) => ({ ...prev, scanning: false }));
            });
        }
      }, 200);
    },
