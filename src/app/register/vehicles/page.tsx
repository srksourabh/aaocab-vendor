...
import { type PhotoAssessment } from "@/lib/ai/document-parser";
...
export default function VehiclesPage() {
  ...
  const handlePhotoCapture = useCallback(
    (
      ...
      fetch("/api/vendor/assess-photo", { method: "POST", body: fd })
        .then((res) => res.json())
        .then((assessment) => {
          setVehicles((prev) =>
            prev.map((v, i) => {
        ...
        .catch(() => {
          setVehicles((prev) =>
            prev.map((v, i) => {
              ...
              return {
                ...v,
                photoAI: { ...v.photoAI, [photoType]: { scanning: false, assessment: null } },
              };
            })
          );
        });
